...
if __name__ == '__main__':
    ...
    for act_fun in [Sigmoid, Relu]:
        ...
        for i in range(10):
            print(f"Attempt {i}")
            initializer = GaussianWeightsInitStrategy(standard_dev=10)
            model = Network(input_size=784, learning_step=0.1, gradient_clip=1, optimizer=Adam())
            model.add_layer(13, act_function=act_fun, weights_init_strategy=initializer)
            model.add_layer(12, act_function=act_fun, weights_init_strategy=initializer)
            model.add_layer(11, act_function=act_fun, weights_init_strategy=initializer)
            model.compile(10, weights_init_strategy=initializer)
            verbose = i == 0
